...
def main():
    ...
    parser.add_argument(
        "--npz", required=True, help="predictions.npz from ensemble_skill.py"
    )
    ...
    ax.fill_between(
        days, mean_mhw - std_mhw, mean_mhw + std_mhw, alpha=0.25, color="#d7191c"
    )
    ax.fill_between(
        days,
        mean_nonmhw - std_nonmhw,
        mean_nonmhw + std_nonmhw,
        alpha=0.25,
        color="#2c7bb6",
    )
    ...
    ax.plot(
        ...
        color="#d7191c",
        lw=2.5,
        label=f"MHW events (top-{len(idx_mhw)})",
    )
    ax.plot(
        days,
        mean_nonmhw,
