    ...
    for lm, full_ds, lat, lon in models_datasets:
        ...
        for k, idx in enumerate(idx_events):
            ...
            cam, _ = engine.compute(
                xs.unsqueeze(0).to(device), xt.unsqueeze(0).to(device)
            )
            cam_accum += cam
...
def main():
    ...
    def mask(cam):
        return np.where(coast_mask, np.nan, cam)
    ...
    titles_row1 = [
        f"MHW events (top-{args.n_events})",
    ...
    for col, (cam, title) in enumerate(
        zip([cam_mhw, cam_nomhw, cam_diff], titles_row1)
    ):
        ax = axes[0, col]
        if col < 2:
            im = ax.imshow(
        ...
        plt.colorbar(im, ax=ax, fraction=0.035, pad=0.03).ax.tick_params(
    ...
    fig.suptitle(
        "Grad-CAM spatial attribution — ensemble 4 seeds · 7-day lead",
