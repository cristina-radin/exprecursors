#!/usr/bin/env python
"""
Per-year IG variable importance heatmap — spatial inputs only.

Identical logic to poster_ig_peryear.py but x_temporal is zeroed out before
computing IG, so attributions reflect purely what the model extracts from the
spatial fields (x_spatial) when it has no access to year/month information.

This is the "Level 2 / physical" complement to the full IG analysis:
  - poster_ig_peryear.py          → spatial IG with temporal context active
  - poster_ig_peryear_temporal.py → spatial + temporal IG (year_norm dominates)
  - poster_ig_peryear_spatial.py  → spatial IG, temporal context zeroed out (this file)

Usage:
  python eval/poster_ig_peryear_spatial.py \
    --exp_dirs split_blockyear/TbotAtm_seed100 ... seed400 \
    --npz      split_blockyear/TbotAtm_ensemble/eval_results/predictions.npz \
    --output   poster_figures/fig_ig_peryear_spatial.png \
    --n_per_year 5
"""
...
def main():
    ...
    parser.add_argument("--output",     default="poster_figures/fig_ig_peryear_spatial.png")
    ...
    for yr in all_years:
        ...
        for lm, full_ds in zip(models, datasets):
            for idx in idx_yr:
                xs, xt, _ = full_ds[idx]
                # Zero out temporal context so IG reflects spatial fields only
                xt_zeroed = torch.zeros_like(xt)
                attrs = _integrated_gradients(
                    lm,
                    xs.unsqueeze(0).to(device),
                    xt_zeroed.unsqueeze(0).to(device),
                )
    ...
    mat = np.array([year_importance[y] for y in yrs_with_data])
    ...
    ax.set_title(
        f"Variable importance per year — Integrated Gradients (spatial only, temporal context zeroed)\n"
    ...
    np.savez(
        out.parent / "ig_peryear_spatial.npz",
        years=np.array(yrs_with_data),
        importance=mat,
        variables=np.array(variables),
    )
    print(f"Saved: {out.parent / 'ig_peryear_spatial.npz'}")
