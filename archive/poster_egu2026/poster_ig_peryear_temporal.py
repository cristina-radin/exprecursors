...
def main():
    ...
    parser.add_argument(
        "--output", default="poster_figures/fig_ig_peryear_temporal.png"
    )
    ...
    for yr in all_years:
        ...
        print(
            ...
            + "  ".join(
                f"{v}={year_importance[yr][i]:.1f}%" for i, v in enumerate(all_row_vars)
            )
        )
    ...
    im0 = axes[0].imshow(
        mat[:, :n_vars].T, aspect="auto", cmap="YlOrRd", vmin=0, vmax=vmax
    )
    axes[0].set_xticks([])
