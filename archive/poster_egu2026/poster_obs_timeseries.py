...
def main():
    ...
    parser.add_argument("--nc",     default="/p/project1/hai_1127/inputs/daily/preprocess_data/merged_daily.nc")
    ...
    print(f"  Monthly MHW days range: {monthly_mhw.min():.1f} – {monthly_mhw.max():.1f}")
    ...
    for ax in (ax1, ax2):
        ax.axvspan(ref_start, ref_end, color="lightgrey", alpha=0.4, lw=0, zorder=0,
    ...
    ax1.plot(t, trend_anom.values, color="#1a1a6e", lw=2.0, label="5-yr smoothed trend", zorder=3)
    ...
    ax1.set_title("Basin-mean Sea Temperature Anomaly (8 m)", fontsize=FONTSIZE + 1, fontweight="bold")
    ...
    ax2.plot(t, trend_mhw.values, color="#1a1a6e", lw=2.0, label="5-yr smoothed trend", zorder=3)
    ...
    ax2.set_title("Basin-mean MHW Monthly Frequency (8 m)", fontsize=FONTSIZE + 1, fontweight="bold", pad=8)
