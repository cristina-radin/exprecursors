...
OUT_DIR   = Path("/p/project1/hai_1127/radin1/exprecursors/experiments/figures/causal")
...
def ccm_rho(X: np.ndarray, Y: np.ndarray, E: int, tau: int,
...
def ccm_test(X: np.ndarray, Y: np.ndarray, E: int = CCM_E, tau: int = CCM_TAU,
             n_lib: int = 10, n_boot: int = 20, seed: int = 42) -> tuple:
    """
...
def main():
    ...
    for drv in drivers:
        ...
        sig_g   = "***" if g_pval < 0.001 else ("**" if g_pval < 0.01 else ("*" if g_pval < 0.05 else "ns"))
        sig_ccm = "YES" if ccm_conv else "no"
        print(f"{drv:<12} {g_pval:>10.4f} {g_lag:>12d} {ccm_rho_max:>10.3f} {sig_ccm:>10}  {sig_g}")
    ...
    for ax, method_idx, title, fmt in [
        ...
        im   = ax.imshow(data, cmap="YlOrRd", vmin=0,
                         vmax=4 if method_idx == 0 else 1, aspect="auto")
    ...
    fig.suptitle("Causal triangulation: Granger + CCM\nTarget = NS to_anom", fontsize=12)
    plt.tight_layout()
