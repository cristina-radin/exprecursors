...
im = ax.imshow(mhw_per_yr, origin="lower", extent=extent,
               cmap="YlOrRd", aspect="auto", vmin=0)
plt.colorbar(im, ax=ax, label="MHW days per year", fraction=0.03)
ax.set_title(f"Mean Marine Heatwave days per year ({years[0]}–{years[-1]})\n"
             "Hobday 2016 definition", fontsize=11)
...
ax.set_title(f"Seasonal cycle of MHW frequency — North Atlantic\n"
             f"Mean {years[0]}–{years[-1]} (Hobday 2016)", fontsize=10)
