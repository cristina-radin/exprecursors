...
def main():
    ...
    for fold in range(N_FOLDS):
        ...
        r1_all.append(r1); r60_all.append(r60); r_ns_all.append(rns)
        print(f"  Fold {fold} (n={len(test_idx)} samples):  "
              f"remote_1d r={r1:.4f}  remote_60d r={r60:.4f}  NS_persist r={rns:.4f}")

    print(f"\n{'='*60}")
    print(f"Remote SST 1-day persistence:   r = {np.mean(r1_all):.4f} ± {np.std(r1_all):.4f}")
    print(f"Remote SST 60-day mean:         r = {np.mean(r60_all):.4f} ± {np.std(r60_all):.4f}")
    print(f"NS local persistence:           r = {np.mean(r_ns_all):.4f} ± {np.std(r_ns_all):.4f}")
    print(f"\nMasked model (reference):       r = 0.807 ± 0.038")
    print(f"\nIf masked model r >> remote baselines → model extracts real non-local signal.")
    print(f"If masked model r ≈ remote persistence → correlation only.")

    # Save as txt
    with open(out_dir / "persistence_remote_sst.txt", "w") as f:
        f.write(f"Remote SST 1-day persistence:   r = {np.mean(r1_all):.4f} ± {np.std(r1_all):.4f}\n")
        f.write(f"Remote SST 60-day mean:         r = {np.mean(r60_all):.4f} ± {np.std(r60_all):.4f}\n")
        f.write(f"NS local persistence:           r = {np.mean(r_ns_all):.4f} ± {np.std(r_ns_all):.4f}\n")
        f.write(f"Masked model (reference):       r = 0.807 ± 0.038\n")
