...
def main():
    ...
    for lead in sorted(groups):
        ...
        leads.append(lead)
        r_mean.append(np.mean(rs));  r_std.append(np.std(rs))
    ...
    ax.fill_between(leads, r_mean - r_std, r_mean + r_std,
                    alpha=0.2, color="#2166ac", label="±1 std (25 runs)")
    ...
    for x, y in zip(leads, r_mean):
        ax.annotate(f"{y:.2f}", (x, y), textcoords="offset points",
                    xytext=(0, 9), ha="center", fontsize=8.5, color="#2166ac")

    # --- MAE ---
    ax = axes[1]
    ax.plot(leads, mae_mean, "s-", color="#d6604d", lw=2, ms=7, zorder=3)
    ax.fill_between(leads, mae_mean - mae_std, mae_mean + mae_std,
                    alpha=0.2, color="#d6604d", label="±1 std (25 runs)")
    ax.set_xlabel("Lead time (days)", fontsize=12)
    ...
    for x, y in zip(leads, mae_mean):
        ax.annotate(f"{y:.2f}", (x, y), textcoords="offset points",
                    xytext=(0, 9), ha="center", fontsize=8.5, color="#d6604d")
