...
def main():
    ...
    parser.add_argument("--config",    default="split_blockyear/TbotAtm_seed100/config.yaml")
    ...
    N        = len(ens)
    date_arr = np.array([
        times[i + full_ds.window_size - 1 + full_ds.lead_time]
        for i in range(N)
    ])
    ...
    for i, (ax, (yr_start, yr_end)) in enumerate(zip(axes, decades)):
        ...
        ax.fill_between(date_arr[mask],
                        ...
                        ens[mask] + seed_std[mask],
                        color="#e08080", alpha=0.30, lw=0)
        ax.fill_between(date_arr[mask], -4, 4,
                        where=(mhw_mask[mask] == 1),
                        transform=ax.get_xaxis_transform(),
                        alpha=0.12, color="gold", lw=0)
        l1, = ax.plot(date_arr[mask], trues[mask],
                      color="#2c7bb6", lw=1.2, alpha=0.9, label="Observation")
        l2, = ax.plot(date_arr[mask], ens[mask],
                      color="#d7191c", lw=1.2, alpha=0.85, ls="--",
        ...
        ax.text(0.01, 0.93, f"{yr_start}–{yr_end}",
    ...
    fig.text(0.08, 0.53, "SST anomaly (norm.)", fontsize=FONTSIZE,
