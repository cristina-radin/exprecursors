...
def main():
    ...
    # Get split info to label test set
    test_indices = set(dm.test_dataset.indices) if hasattr(dm, "test_dataset") else set()
    val_indices  = set(dm.val_dataset.indices)  if hasattr(dm, "val_dataset")  else set()
    ...
    def report_metrics(label, mask):
        ...
        print(f"  POD={pod_:.4f}  FAR={far_:.4f}  CSI={csi_:.4f}  ETS={ets_:.4f}  r={r_:.4f}")
    ...
    for yr, n_mhw, p, f, c, e, rr, sp in rows:
        print(f"{yr:>5}  {sp:>5}  {n_mhw:>4}  {p:>5.3f}  {f:>5.3f}  {c:>5.3f}  {e:>5.3f}  {rr:>5.3f}")
    ...
    with open(txt_path, "w") as fp:
        ...
        def write_split(label, mask):
            ...
            fp.write(f"  N={mask.sum()}  MHW_truth={H_+M_} ({100*(H_+M_)/mask.sum():.1f}%)\n")
            fp.write(f"  H={H_}  FA={FA_}  M={M_}  CN={CN_}\n")
            fp.write(f"  POD={pod_:.4f}  FAR={far_:.4f}  CSI={csi_:.4f}  ETS={ets_:.4f}  r={r_:.4f}\n\n")

        write_split("TRAIN", split_mask == "train")
        write_split("VAL",   split_mask == "val")
        write_split("TEST",  split_mask == "test")

        fp.write(f"OVERALL (all splits)\n")
        fp.write(f"  N={len(trues)}  MHW_truth={H+M} ({100*(H+M)/len(trues):.1f}%)  MHW_pred={H+FA} ({100*(H+FA)/len(preds):.1f}%)\n")
        fp.write(f"  H={H}  FA={FA}  M={M}  CN={CN}\n")
        fp.write(f"  POD={pod:.4f}  FAR={far:.4f}  CSI={csi:.4f}  ETS={ets:.4f}  r={r:.4f}\n\n")
        fp.write(f"{'Year':>5}  {'Split':>6}  {'MHW':>4}  {'POD':>6}  {'FAR':>6}  {'CSI':>6}  {'ETS':>6}  {'r':>6}\n")
        for yr, n_mhw, p, f, c, e, rr, sp in rows:
            fp.write(f"{yr:>5}  {sp:>6}  {n_mhw:>4}  {p:>6.4f}  {f:>6.4f}  {c:>6.4f}  {e:>6.4f}  {rr:>6.4f}\n")
    print(f"\nSaved: {txt_path}")
    ...
    csi_arr = np.array([r[4] for r in rows])
    ets_arr = np.array([r[5] for r in rows])
    r_arr   = np.array([r[6] for r in rows])
    mhw_arr = np.array([r[1] for r in rows])
    ...
    ax.step(yr_arr, pod_arr, color="k", lw=0.6, ls="--", where="mid", label="POD (dashed)")
    ax.axhline(0, color="k", lw=0.8)
    ax.set_ylabel("ETS / POD")
    ax.set_title("Per-year skill scores — binary MHW (to_anom > 0, 7-day lead)\n"
    ...
    fig.suptitle("Prediction vs truth — full period (7-day lead)\n"
                 "background: blue=train  orange=val  red=test", fontsize=11, y=1.01)

    split_bg = {"train": ("steelblue", 0.07), "val": ("darkorange", 0.13), "test": ("crimson", 0.20)}
    ...
    for ax, (yr_start, yr_end) in zip(axes, decades):
        ...
        for yr in range(yr_start, yr_end + 1):
            ...
            ax.axvspan(np.datetime64(f"{yr}-01-01"), np.datetime64(f"{yr+1}-01-01"),
                       color=color, alpha=alpha, lw=0)

        ax.plot(date_arr[mask], trues[mask],
                color="steelblue", lw=0.9, label="Truth", alpha=0.9)
        ax.plot(date_arr[mask], preds[mask],
                color="tomato", lw=0.9, label="Prediction", alpha=0.85, ls="--")
    ...
    ax.axhline(y_true_bin.mean(), color="gray", ls=":", lw=1, label=f"Climatology ({y_true_bin.mean():.2f})")
