"""
quantile_head_recall_v2_fold0.py — early sanity check, Aug 21 2026: same
decisive check as quantile_head_recall.py (which found v1's quantile
head massively beats the mean head, see narrative.md Aug 20-21 entry),
but run against full_gnll_quantile_v2's fold0 ONLY, while the array job
for folds 1-4 (29426208) is still training. Cheap (~5-8 min, one fold,
checkpoint already on disk from job 29417405), so no reason to wait for
all 5 folds to finish before getting an early read on whether v2's
quantile head still shows the same strong signal as v1's did.
...
print(
    "\n=== full_gnll_quantile_v2 fold0 ONLY (early check while folds 1-4 still training) ==="
)
for fold in [0]:
...
print(
    "\n=== SUMMARY: full_gnll_quantile_v2 fold0 ONLY, mean head vs quantile head (tau=0.9) ==="
)
print(
...
np.savez(
    FIGURES_DIR / "_fold_cache" / "quantile_head_check_v2_fold0.npz",
    trues_c=trues_c,
    mean_c=mean_c,
    q_c=q_c,
    thresh1=thresh1,
...
print(
    f"\nSaved raw arrays to {FIGURES_DIR / '_fold_cache' / 'quantile_head_check_v2_fold0.npz'}"
)
