...
def main():
    ...
    ax.legend()
    plt.tight_layout()
    fig.savefig(output_dir / "composite_ig_importance.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: composite_ig_importance.png")
    ...
    for i, var in enumerate(variables):
        ...
        for ax, data, title, cmap, vmin, vmax in [
            ...
            (axes[1], sp1,  f"non-MHW  ({len(nomhw_sample)} samples)", "YlOrRd", 0,         vmax_abs),
            (axes[2], diff, "Difference (MHW minus non-MHW)",         "RdBu_r", -vmax_diff, vmax_diff),
        ]:
        ...
        plt.tight_layout()
        fig.savefig(output_dir / f"composite_ig_spatial_{var}.png",
    ...
    for j, (ax, var) in enumerate(zip(axes, variables)):
        for gname, temporal in temporal_by_group.items():
            ax.plot(days, temporal[:, j], label=gname,
                    lw=1.8, color=colors.get(gname))
    ...
    plt.close(fig)
    print(f"Saved: composite_ig_temporal.png")
