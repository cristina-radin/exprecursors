...
def main():
    ...
    with torch.no_grad():
        for idx in idx_events:
            ...
            all_lstm_stds.append(lstm_out.squeeze(0).std(dim=-1).cpu().numpy())  # (window,)
    ...
    print(f"  Mean std across hidden dim, averaged over events:")
    print(f"  min={mean_std_per_t.min():.4f}  max={mean_std_per_t.max():.4f}  "
          f"range={mean_std_per_t.max()-mean_std_per_t.min():.4f}")
    if mean_std_per_t.max() - mean_std_per_t.min() < 0.01:
        print("  ⚠ LSTM hidden states nearly identical across timesteps → collapsed LSTM")
    else:
        print("  ✓ LSTM outputs vary across timesteps")

    print("\n── 2. Raw attention scores (before softmax) ──")
    score_ranges = all_scores.max(axis=1) - all_scores.min(axis=1)
    print(f"  Per-event score range:  mean={score_ranges.mean():.4f}  "
    ...
    print(f"  Weight std over time:   mean={weight_stds.mean():.5f}  "
          f"(uniform would be {np.sqrt(uniform*(1-uniform)/window):.5f})")
    print(f"  Max weight per event:   mean={all_weights.max(axis=1).mean():.4f}  "
    ...
    with torch.no_grad():
        for idx in idx_events:
            ...
            w_last  = torch.zeros(window, device=device); w_last[-7:] = 1/7
            w_first = torch.zeros(window, device=device); w_first[:7] = 1/7
    ...
    if np.mean(diffs_last) < 0.05 and np.mean(diffs_first) < 0.05:
        ...
        print("    Diagnosis: LSTM already integrates the full sequence; attention is decorative")
    ...
    print("  Max weight mean: ", round(all_weights.max(axis=1).mean(), 4),
