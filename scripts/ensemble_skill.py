...
def skill_scores(H, FA, M, CN):
    ...
    hss = (
        2 * (H * CN - FA * M) / ((H + M) * (M + CN) + (H + FA) * (FA + CN))
        if ((H + M) * (M + CN) + (H + FA) * (FA + CN)) > 0
        else np.nan
    )
...
def main():
    ...
    for exp_dir in exp_dirs:
        ...
        print(
            f"  r={pearson_r(preds_i, np.array([full_ds[i][2].item() for i in range(len(full_ds))])):.4f}"
        )
    ...
    for i in range(N):
        test_preds = [
            all_preds[s][i] for s in range(n_seeds) if all_splits[s][i] == "test"
        ]
        if test_preds:
            ...
            val_preds = [
                all_preds[s][i] for s in range(n_seeds) if all_splits[s][i] == "val"
            ]
            if val_preds:
    ...
    with open(txt_path, "w") as fp:
        ...
        def report(label, mask, pred_arr):
            ...
            print_and_write(
                fp,
                f"  POD={pod:.4f}  FAR={far:.4f}  CSI={csi:.4f}  ETS={ets:.4f}  HSS={hss:.4f}  r={r:.4f}",
            )
            # per-year
            print_and_write(
                fp,
            ...
            for yr in sorted(set(years[mask])):
                ...
                print_and_write(
                    fp,
                    f"  {yr:>5}  {ymask.sum():>5}  {pod_:>6.3f}  {far_:>6.3f}  {csi_:>6.3f}  {ets_:>6.3f}  {hss_:>6.3f}  {r_:>6.3f}",
                )
        ...
        for s, (exp_dir, preds_s, splits_s) in enumerate(
            zip(exp_dirs, all_preds, all_splits)
        ):
            ...
            print_and_write(
                fp,
                f"  seed={seed:>4}  N={test_mask.sum():>5}  POD={pod:.4f}  FAR={far:.4f}  CSI={csi:.4f}  ETS={ets:.4f}  HSS={hss:.4f}  r={r:.4f}",
            )
    ...
    for s, (exp_dir, preds_s) in enumerate(zip(exp_dirs, all_preds)):
        ...
        ax.plot(
            fpr_, tpr_, lw=1, alpha=0.5, ls="--", label=f"seed={seed} (AUC={auc_:.3f})"
        )
    ...
    date_arr = np.array(
        [times[idx + full_ds0.window_size - 1 + full_ds0.lead_time] for idx in range(N)]
    )
    ...
    fig.suptitle(
        f"Ensemble prediction vs truth — {n_seeds} seeds  (7-day lead)", fontsize=11
    )
