...
def main():
    ...
    print(f"  p90_ns: min={p90_ns.min():.3f}  max={p90_ns.max():.3f}  mean={p90_ns.mean():.3f} °C")
    ...
    print(f"Exceedance days: {exceedance.sum()}  |  After Hobday: MHW={len(mhw_idx)}  non-MHW={len(nomhw_idx)}")
    ...
    ax.set_title(f"Signed variable importance: MHW vs non-MHW (Hobday)\n"
    ...
    for i, var in enumerate(variables):
        ...
        plt.tight_layout()
        fig.savefig(output_dir / f"hobday_ig_spatial_{var}.png", dpi=150, bbox_inches="tight")
        plt.close(fig)
