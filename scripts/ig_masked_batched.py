...
def compute_ig_batch(lm, xs_batch: torch.Tensor, xt_batch: torch.Tensor,
    ...
    for step in range(n_steps):
        ...
        pred[:, 0].sum().backward()  # grad correct per-sample: d(sum pred) / d(interp_i) = d(pred_i)/d(interp_i)
...
def main():
    ...
    parser.add_argument("--seed",       type=int, required=True,
                        choices=ALL_SEEDS, help="Seed to process (one job per seed)")
    ...
    print(f"Seed={args.seed}  device={device}  batch_size={args.batch_size}  n_steps={args.n_steps}")
    ...
    for fold in range(N_FOLDS):
        ...
        indices  = list(test_ds.indices if hasattr(test_ds, "indices") else range(len(test_ds)))
        ...
        for batch_start in range(0, len(indices), B):
            ...
            for gi in batch_gidx:
                ...
                xs_list.append(xs); xt_list.append(xt); yr_list.append(yr)
            ...
            attrs = compute_ig_batch(lm, xs_batch, xt_batch, args.n_steps)  # (B,T,C,H,W)
