...
def main():
    ...
    parser.add_argument("--output_dir", default="experiments/figures/xai_casestudy_2023")
    ...
    for seed in SEEDS:
        ...
        for local_i, global_i in enumerate(indices):
            ...
            if (local_i + 1) % 50 == 0:
                print(f"  {local_i+1}/{len(indices)}  mhw={n_mhw_this}  base={n_base_this}", end="\r")
    ...
    if n_mhw == 0:
        print(f"WARNING: No MHW {MHW_YEAR} Jun-Jul samples found in fold={TARGET_FOLD} test set!")
    ...
    # Save
    np.savez(str(out_dir / "casestudy_signed_ig.npz"),
             mhw_mean=mhw_mean, base_mean=base_mean, diff_mean=diff_mean,
    ...
    for ci, var in enumerate(variables):
        ...
        plt.tight_layout()
        fig.savefig(str(out_dir / f"casestudy_signed_ig_{var}.png"), dpi=150, bbox_inches="tight")
        plt.close()
    ...
    ax.bar(x - width/2, mhw_var_imp  / mhw_var_imp.sum()  * 100,
           width, label=f"MHW Jun–Jul {MHW_YEAR}", color="tomato", alpha=0.85)
    ax.bar(x + width/2, base_var_imp / base_var_imp.sum() * 100,
           width, label="Climatological baseline", color="steelblue", alpha=0.85)
    ax.set_xticks(x); ax.set_xticklabels(variables, rotation=15)
    ax.set_ylabel("% of total |IG|")
    ax.set_title(f"Variable importance comparison — MASKED model (fold={TARGET_FOLD}, {n_runs} seeds)")
    ax.legend()
    plt.tight_layout()
    ...
    for ci, var in enumerate(variables):
        m = mhw_var_imp[ci]; b = base_var_imp[ci]
