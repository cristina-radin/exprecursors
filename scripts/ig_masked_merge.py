...
def main():
    ...
    if not partial_files:
        print(f"No ig_partial_seed*.npz found in {out_dir}")
        return
    ...
    peryear_avg = np.where(
        peryear_cnt[:, None] > 0, peryear_sum / peryear_cnt[:, None], 0.0
    )
    ...
    ax.set_xlabel("Year")
    ax.set_title(
        f"Variable importance (% |IG|) — MASKED model (NS zeroed)\n"
