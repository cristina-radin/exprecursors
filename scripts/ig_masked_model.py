...
def main():
    ...
    ax.set_xlabel("Year")
    ax.set_title("Variable importance (% |IG|) — MASKED model (NS zeroed)\nOnly remote Atlantic + atmosphere")
    ...
    # ── Spatial maps ──────────────────────────────────────────────────────────
    ds_meta = __import__("xarray").open_dataset(
    ...
    # Mark NS box
    ns_lat = slice(100, 127); ns_lon = slice(150, 187)
    ...
    for ci, var in enumerate(variables):
        ...
        ax.set_title(f"Mean |IG| spatial attribution — {var}\nMASKED model (NS zeroed, {total_runs} runs)")
