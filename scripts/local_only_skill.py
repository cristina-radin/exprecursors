...
def run_conditions(lm, test_ds, device, batch_size=32):
    ...
    with torch.no_grad():
        for b_start in range(0, len(indices), batch_size):
            ...
            for gi in batch_idx:
                xs, xt, y = base_ds[gi]
                xs_list.append(xs); xt_list.append(xt); y_list.append(y)
