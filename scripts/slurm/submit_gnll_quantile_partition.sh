...
# changed. The #SBATCH directives, module loads, and `--partition=booster`
# below are JUWELS-specific; if submitting on a different cluster, replace
# those (module names, account/partition, GPU request syntax) for that
# system before use. Fold0 of the plain full_gnll run took ~1h01m wall time
# on JUWELS (same architecture minus the aux head, which adds negligible
# compute) — --time=04:00:00 has ample margin either way.
#
