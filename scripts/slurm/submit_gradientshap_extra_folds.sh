#!/bin/bash
# Aug 22 2026: GradientSHAP on fold1/fold2, both committed and nearest.
...
#SBATCH --job-name=gradientshap_extra_folds
#SBATCH --array=0-3
...
echo "GradientSHAP: ${LABEL}  config=configs/partition/${CFG_DIR}/fold${FOLD}.yaml"
echo "Start: $(date)"
python -u scripts/gradientshap_quantile_partition.py \
    --config configs/partition/${CFG_DIR}/fold${FOLD}.yaml \
    --output experiments/figures/xai_gradientshap/gradientshap_quantile_v2_${LABEL} \
