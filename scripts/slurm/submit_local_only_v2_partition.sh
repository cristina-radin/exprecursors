#!/bin/bash
# Item 2 (Aug 21 2026): local-only masking, full 5-fold array, same
...
# via --mode local_only (zeros everything outside the NS box), NOT via
# the yaml.
#
#   sbatch --account=mmm_gpu --mail-user=cristina.radin@uni-hamburg.de scripts/slurm/submit_local_only_v2_partition.sh
...
CFG="${REPO_DIR}/configs/partition/local/fold${SLURM_ARRAY_TASK_ID}.yaml"

echo "local_only_v2 Fold: ${SLURM_ARRAY_TASK_ID}  Config: ${CFG}"
echo "Start: $(date)"

cd "${REPO_DIR}"
python -u scripts/train_partition.py --config "${CFG}" --mode local_only
